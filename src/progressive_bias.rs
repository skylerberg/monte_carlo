use crate::monte_carlo::{upper_confidence_bound, MonteCarloTreeNode, MonteCarloTreeSearch};
use crate::Game;
...
impl<G: Game> MonteCarloTreeSearch for ProgressiveBiasPolicy<G> {
    ...
    fn get_selection_value(
        ...
        upper_confidence_bound(cumulative_reward, games, total_game_count, c)
            + (self.heuristic_function)(&cloned_game, child.player_id) / (child.games + 1.0)
    }
}
