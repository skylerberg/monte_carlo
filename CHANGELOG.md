...
* **An over-declared reward range no longer dissolves `Duct`'s answer.** The tie
  tolerance is measured against the spread of the payoffs the search has
  collected, falling back to the declared range until `MIN_EVIDENCE` of them
  have arrived, rather than against the declaration outright. Declaring
  `[0, 200]` for a game paying in `[0, 1]` used to widen the tie pool two
  hundredfold until every arm was in it: on a two-arm game with a strictly
  dominant row, 0.506 of the visits landed on the dominant arm — chance. It is
  above 0.9 now, matching what the correctly-declared range gives. The previous
  release documented this as a known limitation on the grounds that the spread
  of the *values at a node* is the wrong reference — which it is, since one arm
  with a bad estimate would widen the band the good arms are compared inside.
  The spread of the *payoffs* is a different quantity and is the right one.
